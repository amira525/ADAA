...
jQuery(document).ready(function($){
  ...
  function Calendar(element, Args) {
    ...
    this.conf = {
      ...
      event: {
        hover: {
          delay: (Args.event) ? (Args.event.hover) ? (Args.event.hover.delay) ? Args.event.hover.delay : 95 : 95 : 95
        }
      },
      colors: {
      ...
      categories: {
        ...
        hover: {
          delay: (Args.categories) ? (Args.categories.hover) ? (Args.categories.hover.delay) ? Args.categories.hover.delay : 95 : 95 : 95
        }
      },
      now: {
  ...
  Calendar.prototype.weekDrawTime = function() {
    ...
    var i = 0;
    var n= this.conf.weekday.timeline.fromHour+1;
    while (i < limit){
      li = $('<li>');
      li.append($('<span>').text(time.format(this.conf.weekday.timeline.format)));
      li.height(this.conf.weekday.timeline.heightPx-20);
      ul.append(li);
  ...
  Calendar.prototype.weekDrawDays = function() {
    ...
    for (var i=0; i<days.length; i++){
      ...
      if (i == 0 && this.mobileQuery() == 'mobile'){
        div.append($('<button>', {
          class: 'btn btn-sm btn-default btn-move-calendar-left'
      ...
      if (i == days.length - 1 && this.mobileQuery() == 'mobile'){
        div.append($('<button>', {
          class: 'btn btn-sm btn-default btn-move-calendar-right'
  ...
  Calendar.prototype.setEventPosition = function(elem, eventTop, eventHeight, eventLeft, eventWidth) {
    $(elem).attr('data-top', eventTop*2/3);
    $(elem).attr('data-height', 30);
  ...
  Calendar.prototype.drawEventsOrDaynotes = function(e, index, start, end, title, content, category, color, time_interval, classes) {
    ...
    //   var e= $('.calendar-event').addClass('hidden');
    //   e._rev = response.rev;
  ...
  Calendar.prototype.addBtnLeft = function() {
    var self = this;
    this.element.find('.btn-move-calendar-left').click(function(event){
      elem = $(event.target);
      if (elem.prop('nodeName') !== 'BUTTON'){
        elem = elem.closest('.btn-move-calendar-left');
      }
      self.moveCurrentIntervalView($(elem).attr('data-direction'));
    });
  };
  Calendar.prototype.addBtnRight = function() {
    var self = this;
    this.element.find('.btn-move-calendar-right').click(function(event){
      elem = $(event.target);
      if (elem.prop('nodeName') !== 'BUTTON'){
        elem = elem.closest('.btn-move-calendar-right');
      }
      self.moveCurrentIntervalView($(elem).attr('data-direction'));
    });
  };
  ...
  $(document).ready(function(){
    ...
    if(document.querySelector('.choose').value == '1'){
      document.querySelector('li.calendar-event').style.backgroundColor = Calendar.conf.colors.events;
    }
    if(document.querySelector('.choose').value == '2'){
      document.querySelector('li.calendar-event').style.backgroundColor = Calendar.conf.colors.daynotes;
    }
    ...
    $('#calendar').unbind('Calendar.event-click').on('Calendar.event-click', function(event, instance, elem, evt){
      ...
      $('.calendar-month-events-day').modal('show');
    });
    ...
    $('#form-update').on('submit', function(event){
      ...
      eventsDB.put(e).then(function(response){
        ...
        Calendar.init();
        $('.calendar-month-events-day').modal('hide');
      }).catch(function(err){
        console.error(err);
      });
    });
